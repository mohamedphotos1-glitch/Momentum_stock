...
Placez votre fichier d'univers (par défaut `univers_us.csv`) dans le dossier `data/`. Le fichier peut être un CSV **ou un Excel (.xlsx)** tant qu'il contient une colonne de symboles (`Symbol`, `NASDAQ Symbol` ou `ACT Symbol`). Un fichier d'exemple minimal est fourni (`data/univers_us.csv`) pour vérifier rapidement que tout fonctionne.
